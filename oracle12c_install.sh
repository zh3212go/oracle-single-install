...
HostIP="192.168.56.99"
HostName='oracle12c'
#oracle user password
OracleUserPasswd="oracle123"
#default `systemOracle.com`
ORACLE_DB_PASSWD="oracledb123"
...
function base_config() {
  ...
  if [[ ${HostName} != 'DB' ]];then
    sed -i "s/ORACLE_HOSTNAME=DB/ORACLE_HOSTNAME=${HostName}/g" /tmp/oracleInstallTmp.txt
  fi
  if [[ ${characterSet} != '' ]];then
    sed -i "s/NLS_LANG=american_america.zhs16gbk/NLS_LANG=american_america.${characterSet}/g" /tmp/oracleInstallTmp.txt
  fi
...
function oracle_file() {
  ...
  if [[ ${characterSet} != '' ]];then
     ...
     sed -i "s/characterSet=AL32UTF8/characterSet=${characterSet}/g" dbca_single.rsp
  fi
  sed -i "s#datafileDestination={ORACLE_BASE}/fast_recovery_area/{DB_UNIQUE_NAME}#datafileDestination={ORACLE_BASE}/oradata/{DB_UNIQUE_NAME}#g" dbca_single.rsp
